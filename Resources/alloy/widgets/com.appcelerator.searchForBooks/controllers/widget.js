function WPATH(s) {
    var index = s.lastIndexOf("/"), path = index === -1 ? "com.appcelerator.searchForBooks/" + s : s.substring(0, index) + "/com.appcelerator.searchForBooks/" + s.substring(index + 1);
...
function Controller() {
    ...
    $.__views.text = A$(Ti.UI.createTextField({
        top: "3dp",
        bottom: "1dp",
        left: "7dp",
        right: "60dp",
        ...
        id: "text"
    }), "TextField", $.__views.bar);
    $.__views.bar.add($.__views.text);
    $.__views.text.on("return", searchForBooks);
    ...
    $.__views.searchView.on("click", searchForBooks);
    $.__views.search = A$(Ti.UI.createImageView({
        height: Ti.UI.FILL,
        width: Ti.UI.FILL,
        color: "#fff",
    ...
    model.on("change:loading", function(m) {
        if (m.get("loading")) {
            $.searchView.touchEnabled = !1;
            $.search.opacity = 0;
            $.loading.setOpacity(1);
        } else {
            $.loading.setOpacity(0);
            $.search.opacity = 1;
            $.searchView.touchEnabled = !0;
        }
    });
    _.extend($, exports);
}
